...
/** True when two percent-coordinate boxes share any area. */
...
const roomPlanSchema = {
  ...
  properties: {
    ...
    items: {
      ...
      description: "2-3 collectible objects inside, story-flavored.",
    },
    actions: {
      type: Type.ARRAY,
      items: plannedActionSchema,
      description: "2-3 environmental interactions inside. At least one should carry risk.",
    },
  },
...
/**
 * The world document every downstream call referees against. Exported so Vaarta's
 * language planner can author the same shape in one call and reuse this whole
 * screen/interior/dialogue pipeline unchanged.
 */
    ...
    street: {
      ...
      properties: {
        ...
        items: {
          type: Type.ARRAY,
          items: plannedItemSchema,
          description: "2-3 collectible objects lying in the open.",
        },
        actions: {
          type: Type.ARRAY,
          items: plannedActionSchema,
          description: "2-3 environmental interactions in the open.",
        },
      },
...
const screenVisionSchema = {
  ...
  properties: {
    ...
    obstacles: {
      type: Type.ARRAY,
      description:
        "Every SOLID thing in this frame that a person would have to walk around: building and hut footprints, walls and fences, water, rocks, large trees, carts, stalls, crates, wells. One tight box each, up to 14. Do NOT box open ground, paths, grass or paving — those are where the player walks.",
      items: rectSchema,
    },
    items: {
      type: Type.ARRAY,
      description:
        "2-4 small collectible objects actually VISIBLE in the frame (pots, tools, papers, offerings, fruit, rope), story-flavored for this world, each boxed where it sits.",
      items: {
        type: Type.OBJECT,
        properties: {
          name: { type: Type.STRING },
    ...
    actions: {
      type: Type.ARRAY,
      description:
        "2-4 environmental interactions using props actually VISIBLE in the frame (ring, search, peek, light, draw water…). Give suspicion 15-30 to at most one genuinely rash action, 0 to the rest.",
      items: {
        type: Type.OBJECT,
        properties: {
  ...
  required: ["title", "ambient", "edges", "obstacles", "buildings", "items", "actions"],
};

/** A box big enough to wall the player in is a vision mistake, not a wall. */
const MAX_OBSTACLE_AREA = 30;

/**
 * Keep only obstacles that make sense to collide with.
 *
 * The vision pass occasionally boxes the whole frame (it has "found" the
 * ground) or returns slivers with no area. Either one, made solid, ruins the
 * screen: the first traps the player on arrival, the second scatters invisible
 * snags across open paving. Dropping them is always safer than honouring them,
 * because the failure mode of a missing wall is only that you can walk through
 * a cart.
 */
function usableObstacles(raw: Rect[] | undefined): Rect[] {
  return (raw ?? [])
    .map(clampRect)
    .filter((rect) => {
      const area = (rect.w * rect.h) / 100;
      return area >= 0.3 && area <= MAX_OBSTACLE_AREA;
    })
    .slice(0, 14);
}
...
export async function generateScreen(
    ...
    contents: [
      ...
      { inlineData: { data: traced.b64, mimeType: traced.mimeType } },
      {
        text: [
          ...
          "SEPARATELY, fill `obstacles` with the SOLID footprint of everything a walking person would have to go around. This is what stops the player walking through a wall, so be generous and accurate: box every building, wall, water body, rock, big tree, cart, stall and crate. Leave every path, road, courtyard and patch of open grass unboxed — that is the walkable ground.",
        ]
          .filter(Boolean)
          .join("\n"),
      },
  ...
  (spec.actions ?? []).slice(0, 4).forEach((a, i) =>
    hotspots.push({
      id: `${id}-act${i}`,
      kind: "action",
      name: a.name,
  ...
  return {
    ...
    hotspots,
    // Buildings are solid on top of whatever the obstacle pass found: a door
    // you can walk through the wall beside is the single most immersion-
    // breaking thing in a top-down world, and the vision pass boxes buildings
    // reliably enough to make that free.
    obstacles: [
      ...usableObstacles(spec.obstacles),
      ...hotspots.filter((h) => h.kind === "building").map((h) => h.rect),
    ],
    coord: { x, y },
    edges,
  };
...
const interiorLayoutSchema = {
  ...
  properties: {
    ...
    obstacleRects: {
      ...
      items: rectSchema,
      description:
        "4-10 boxes over the SOLID furniture and walls in your image: counters, shelves, tables, stoves, cupboards, beds, and the wall band along the top edge. Never box the open floor, the doorway, or the tile the NPC stands on — the player has to be able to reach both of those.",
    },
  },
  required: [
    "ambient",
...
export async function generateInteriorScene(
  ...
  const npcZone = clampRect(spec.npcZone);
  const exitZone = clampRect(spec.exitZone);
  ...
  return {
    ...
    hotspots,
    // Furniture is solid, but never at the cost of reachability: a box that
    // swallows the doorway or the character locks the player out of the only
    // two things a room exists for.
    obstacles: usableObstacles(spec.obstacleRects).filter(
      (rect) => !overlaps(rect, npcZone) && !overlaps(rect, exitZone)
    ),
    npc: {
      name: npc.name,
      role: npc.role,
...
export async function generateSprite(
  character: { setup: string; styleBible: string },
  referenceFrame: string | null
): Promise<string> {
  const prompt = [
    `Full-body 2D adventure-game player character for this world: ${character.setup}`,
    referenceFrame
